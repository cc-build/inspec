...
module Inspec::Resources
  class MongodbSession < Inspec.resource(1)
    ...
    def initialize(opts = {})
      ...
      fail_resource "Can't run MongoDB checks without authentication" unless database

      @params = run_command(@command)
    end

    def to_s
      "MongoDB Session #{@command}"
    end
    ...
    def run_command(command)
      ...
      @client.command(command).documents.first
    rescue => e
